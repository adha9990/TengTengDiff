...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--validation_prompt_blend",
        type=str,
        default=None,
        help="A prompt that is used during validation to verify that the model is learning.",
    )
    parser.add_argument(
        "--validation_prompt_fg",
        type=str,
        default=None,
        help="A prompt that is used during validation to verify that the model is learning.",
    )
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        text_input_blends = tokenize_prompt(
            self.tokenizer, self.instance_prompt_blend, tokenizer_max_length=self.tokenizer_max_length
        )
        ...
        text_input_fgs = tokenize_prompt(
            self.tokenizer, self.instance_prompt_fg, tokenizer_max_length=self.tokenizer_max_length
        )
...
def main(args):
    ...
    train_dataset = DreamBoothDataset(
        ...
        instance_prompt_blend=args.instance_prompt_blend,
        instance_prompt_fg=args.instance_prompt_fg,
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            pipeline_args = {"prompt": args.validation_prompt, "num_inference_steps": args.num_inference_steps}
            images = log_validation(
                pipeline,
                args,
                accelerator,
