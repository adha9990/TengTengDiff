...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        size=512,
        center_crop=False,
        encoder_hidden_states=None,  # For blend embeddings when pre-computed
        instance_prompt_encoder_hidden_states=None,  # For fg embeddings when pre-computed
        ...
        # When pre-computed, encoder_hidden_states is for blend, instance_prompt_encoder_hidden_states is for fg
        self.encoder_hidden_states_blend = encoder_hidden_states
        self.encoder_hidden_states_fg = instance_prompt_encoder_hidden_states
    ...
    def __getitem__(self, index):
        ...
        # Handle text inputs for blend and fg
        if self.encoder_hidden_states_blend is not None:
            # Pre-computed embeddings for blend
            example["instance_prompt_id_blends"] = self.encoder_hidden_states_blend
        else:
        ...
        if self.encoder_hidden_states_fg is not None:
            # Pre-computed embeddings for fg
            example["instance_prompt_id_fgs"] = self.encoder_hidden_states_fg
        else:
        ...
        if self.class_data_root:
            ...
            if self.encoder_hidden_states_blend is not None:
                # When using pre-computed embeddings, use the same for class
                example["class_prompt_ids"] = self.encoder_hidden_states_blend
            else:
