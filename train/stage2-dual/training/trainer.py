...
class DreamBoothLoRATrainer:
    ...
    def load_models(self):
        """Load tokenizer, text encoder, vae, and unet"""
        # Load the tokenizer
    ...
    def configure_lora(self, unet, text_encoder):
        """Configure LoRA for unet and text encoder"""
    ...
    def setup_hooks(self, unet, text_encoder):
        """Setup save and load hooks for accelerator"""
    ...
    def train(self):
        ...
        # Load models
        tokenizer, text_encoder, vae, unet, noise_scheduler = self.load_models()

        # Freeze models and configure LoRA
        if vae is not None:
            vae.requires_grad_(False)

        self.configure_lora(unet, text_encoder)
        ...
        # Precompute text embeddings if requested
        if self.args.pre_compute_text_embeddings:
            ...
            # Compute embeddings for both blend and fg prompts
            pre_computed_encoder_hidden_states_blend = compute_text_embeddings(
                self.args.instance_prompt_blend
            )
            pre_computed_encoder_hidden_states_fg = compute_text_embeddings(
                self.args.instance_prompt_fg
            )
            ...
            pre_computed_encoder_hidden_states_blend = None
            pre_computed_encoder_hidden_states_fg = None
            validation_prompt_encoder_hidden_states = None
        ...
        train_dataset = DreamBoothDataset(
            ...
            encoder_hidden_states=pre_computed_encoder_hidden_states_blend,  # For blend
            instance_prompt_encoder_hidden_states=pre_computed_encoder_hidden_states_fg,  # For fg
            tokenizer_max_length=self.args.tokenizer_max_length,
        ...
        if self.args.max_train_steps is None:
            ...
            if num_training_steps_for_scheduler != self.args.max_train_steps:
                logger.warning(
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                with self.accelerator.accumulate(unet):
                    pixel_value_blends = batch["pixel_value_blends"].to(
                        dtype=weight_dtype
                    )
                    ...
                    if vae is not None:
                        ...
                        model_input_blend = vae.encode(
                            pixel_value_blends
                        ).latent_dist.sample()
                        model_input_blend = (
                            model_input_blend * vae.config.scaling_factor
                        )
                        model_input_fg = vae.encode(
                            pixel_value_fgs
                        ).latent_dist.sample()
                    ...
                    timesteps = torch.randint(
                        ...
                        (bsz,),
                        device=model_input_blend.device,
                    )
                    timesteps = timesteps.long()

                    # Add noise to the model input according to the noise magnitude at each timestep
                    # (this is the forward diffusion process)
                    noisy_model_input_blend = noise_scheduler.add_noise(
                        model_input_blend, noise_blend, timesteps
                    )
                    noisy_model_input_fg = noise_scheduler.add_noise(
                        model_input_fg, noise_fg, timesteps
                    )

                    # Combine blend and fg for dual training (DualAnoDiff style)
                    noisy_model_input = torch.cat([noisy_model_input_blend.unsqueeze(1), noisy_model_input_fg.unsqueeze(1)], dim=1)
                    noisy_model_input = einops.rearrange(noisy_model_input, 'b f c w h -> (b f) c w h')

                    noise = torch.cat([noise_blend.unsqueeze(1), noise_fg.unsqueeze(1)], dim=1)
                    noise = einops.rearrange(noise, 'b f c w h -> (b f) c w h')

                    model_input = torch.cat([model_input_blend.unsqueeze(1), model_input_fg.unsqueeze(1)], dim=1)
                    model_input = einops.rearrange(model_input, 'b f c w h -> (b f) c w h')

                    # Get the text embedding for conditioning (dual encoding for blend and fg)
                    if self.args.pre_compute_text_embeddings:
                        # For pre-computed embeddings, we need both blend and fg embeddings
                    ...
                    # Combine encoder states for dual training
                    ...
                    # Check if UNet expects concatenated inputs
                    _, channels, _, _ = noisy_model_input_blend.shape
                    if (
                    ...
                    timesteps_dual = torch.cat([timesteps.unsqueeze(1), timesteps.unsqueeze(1)], dim=1)
                    timesteps_dual = einops.rearrange(timesteps_dual, 'b f -> (b f)')

                    if self.args.class_labels_conditioning == "timesteps":
                        class_labels = timesteps_dual
                    else:
                        class_labels = None
                    ...
                    if noise_scheduler.config.prediction_type == "epsilon":
                        ...
                        target = noise_scheduler.get_velocity(
                            model_input, noise, timesteps_dual
                        )
                    else:
                        raise ValueError(
