...
for ANOMALIE in "${ANOMALIES[@]}"; do
    ...
    for CHECKPOINT_STEP in "${CHECKPOINT_STEPS[@]}"; do
        ...
        export LORA_WEIGHTS="all_generate_ai_prompt_mse/$MVTEC_NAME/stage2-$ANOMALIE-dual/checkpoint-$CHECKPOINT_STEP"
        export OUTPUT_DIR="generate_data_ai_prompt_mse/$MVTEC_NAME/stage2-$ANOMALIE-dual/checkpoint-$CHECKPOINT_STEP"
